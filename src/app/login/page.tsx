...
export default function LoginPage() {
  ...
  const quickAccounts: { label: string; name: string; email: string; r: Role }[] = [
    { label: 'Savininkas', name: 'Andrius Kazlauskas', email: 'andrius@mail.lt', r: 'owner' },
    { label: 'Administratorius', name: 'Tomas Domus', email: 'tomas@domus.lt', r: 'admin' },
    { label: 'Darbo vadovas', name: 'Jonas Petraitis', email: 'vadovas@domus.lt', r: 'work_manager' },
    { label: 'Darbininkas', name: 'Tomas Kazlauskas', email: 'darbininkas@domus.lt', r: 'worker' },
  ];
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'fixed', top: -120, left: -120, width: 400, height: 400, borderRadius: '50%', background: 'rgba(12,61,74,0.06)', pointerEvents: 'none' }} />
      <div style={{ position: 'fixed', bottom: -80, right: -80, width: 300, height: 300, borderRadius: '50%', background: 'rgba(232,119,60,0.07)', pointerEvents: 'none' }} />
      ...
      <div style={{ width: '100%', maxWidth: 420, position: 'relative' }}>
        ...
        <div style={{ background: 'var(--color-paper-white)', border: '1px solid var(--color-ghost-border)', borderRadius: 24, overflow: 'hidden', boxShadow: 'var(--shadow-card)' }}>
          ...
          <form onSubmit={handleSubmit} style={{ padding: '24px 24px 28px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: 'var(--color-muted-ash)', marginBottom: 6 }}>El. paštas</label>
              <input
                type="email"
                value={email}
                onChange={e => setEmail(e.target.value)}
                placeholder={role === 'owner' ? 'andrius@mail.lt' : role === 'admin' ? 'tomas@domus.lt' : role === 'work_manager' ? 'vadovas@domus.lt' : 'darbininkas@domus.lt'}
                required
                style={inputStyle}
            ...
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: 'var(--color-muted-ash)', marginBottom: 6 }}>Slaptažodis</label>
              <input
                type="password"
                value={password}
            ...
            {error && (
              <p style={{ fontSize: 13, color: 'var(--color-danger)', background: 'var(--color-danger-tint)', padding: '9px 14px', borderRadius: 10, margin: 0 }}>
                {error}
              </p>
            )}
