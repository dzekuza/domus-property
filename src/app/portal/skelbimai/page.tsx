          ...
          return (
            <button
              ...
              style={{
                ...
                background: active ? (cfg?.color ?? 'var(--color-midnight-ink)') : 'var(--color-cloud-canvas)',
      ...
      {visible.length === 0 ? (
        <Card>
          <div style={{ padding: '48px 24px', textAlign: 'center' }}>
            <p style={{ fontSize: 15, color: 'var(--color-muted-ash-2)', marginBottom: 6 }}>Skelbimų nėra</p>
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '16px 16px 14px', flex: 1 }}>
                  ...
                  {/* Body */}
                  <p style={{ fontSize: 13, color: 'var(--color-muted-ash-2)', lineHeight: 1.6, flex: 1 }}>
                    {post.body}
                  </p>

                  {/* Contact */}
                  {post.contact && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 10px', background: 'var(--color-cloud-canvas)', borderRadius: 8 }}>
                      {post.contact.includes('@')
                        ? <Mail size={13} style={{ color: 'var(--color-muted-ash)', flexShrink: 0 }} />
                        : <Phone size={13} style={{ color: 'var(--color-muted-ash)', flexShrink: 0 }} />
                      }
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderTop: '1px solid var(--color-ghost-border)', background: 'var(--color-cloud-canvas)' }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--color-muted-ash-2)', opacity: 0.7, flexShrink: 0, marginLeft: 8 }}>
        ...
        <DialogContent style={{ maxWidth: 520 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, paddingTop: 4 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 13, fontWeight: 500, color: 'var(--color-midnight-ink)' }}>Kategorija</label>
            ...
            {/* Title */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 13, fontWeight: 500, color: 'var(--color-midnight-ink)' }}>
                Pavadinimas <span style={{ color: 'var(--color-danger)' }}>*</span>
              </label>
              <Input
                placeholder="pvz. Parduodu sofą, Ieškau santechniko…"
                value={form.title}
                onChange={e => { setForm(f => ({ ...f, title: e.target.value })); setErrors(x => ({ ...x, title: undefined })); }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 13, fontWeight: 500, color: 'var(--color-midnight-ink)' }}>
                Aprašymas <span style={{ color: 'var(--color-danger)' }}>*</span>
              </label>
              <Textarea
                placeholder="Detaliau aprašykite skelbimą…"
                rows={4}
                value={form.body}
                onChange={e => { setForm(f => ({ ...f, body: e.target.value })); setErrors(x => ({ ...x, body: undefined })); }}
                style={errors.body ? { borderColor: 'var(--color-danger)', resize: 'vertical' } : { resize: 'vertical' }}
              />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 13, fontWeight: 500, color: 'var(--color-midnight-ink)' }}>
                Kontaktai <span style={{ fontSize: 12, fontWeight: 400, color: 'var(--color-muted-ash-2)' }}>(neprivaloma)</span>
              </label>
              <Input
                placeholder="Telefonas arba el. paštas"
                value={form.contact}
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', paddingTop: 4 }}>
              ...
              <Btn variant="primary" onClick={handleSubmit} disabled={submitting} icon={<Plus size={13} />}>
                Skelbti
              </Btn>
            </div>
          </div>
