    ...
    return (
      <div style={{ minHeight: '100dvh', background: 'var(--background)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--color-muted-ash)' }}>Kreipkitės į darbo vadovą dėl priskyrimo.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="worker-root" style={{ minHeight: '100dvh', background: 'var(--background)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, padding: '24px 20px 32px', display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 540, width: '100%', margin: '0 auto', boxSizing: 'border-box' }}>
        ...
        <div style={{ animation: 'slide-up 0.3s ease' }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--color-muted-ash)', marginTop: 4 }}>Pateikite šios dienos ataskaitą</p>
        </div>
        ...
        {updates.length > 0 && (
          <div style={{ animation: 'slide-up 0.4s ease' }}>
            ...
            >
              <div style={{ width: 32, height: 32, borderRadius: 10, background: 'var(--color-surface-hover)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                {[...updates].reverse().map(update => (
                  <div key={update.id} className="glass" style={{ padding: '12px 18px', borderRadius: 14 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 100, background: update.inputType === 'voice' ? 'rgba(103,205,205,0.15)' : 'rgba(0,0,0,0.06)', color: update.inputType === 'voice' ? 'var(--color-teal)' : 'var(--color-muted-ash)' }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
                        {update.attachments.map(att =>
                          att.mimeType.startsWith('image/') ? (
                            ...
                            <span key={att.id} style={{ fontSize: 11, padding: '3px 8px', background: 'rgba(0,0,0,0.05)', borderRadius: 8, color: 'var(--color-muted-ash)' }}>{att.name}</span>
                          )
                        )}
                      </div>
                    )}
