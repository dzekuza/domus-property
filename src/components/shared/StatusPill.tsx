...
const defectMap: Record<DefectStatus, { label: string; bg: string; color: string }> = {
  ...
  progress: { label: 'Vykdomas',  bg: 'var(--color-violet-tint)',    color: 'var(--color-electric-violet)' },
...
const stepMap: Record<StepStatus, { label: string; bg: string; color: string }> = {
  ...
  progress:   { label: 'Vykdoma',   bg: 'var(--color-violet-tint)',  color: 'var(--color-electric-violet)' },
...
const estateStatusMap: Record<string, { bg: string; color: string }> = {
  Pardavimas: { bg: 'var(--color-violet-tint)', color: 'var(--color-electric-violet)' },
...
export default function StatusPill({ type, value }: Props) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 'var(--radius-pill)', fontSize: 12, fontWeight: 500, background: bg, color }}>
