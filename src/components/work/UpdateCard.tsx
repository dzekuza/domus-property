...
function MiniAudioPlayer({ src }: { src: string }) {
  ...
  return (
    <div style={{
      ...
      {/* Progress track */}
      <div
        style={{ flex: 1, height: 3, background: 'rgba(0,0,0,0.10)', borderRadius: 100, overflow: 'hidden', cursor: 'pointer' }}
        onClick={e => {
      ...
      >
        {selectable && (
          <div style={{
            ...
            background: selected ? 'var(--color-accent)' : 'rgba(0,0,0,0.06)',
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {/* Body text */}
          {update.text && (
            <p style={{ fontSize: 14, color: 'var(--foreground)', lineHeight: 1.65, margin: 0 }}>
              {update.text}
            </p>
          )}
              ...
              {update.attachments.map(att => {
                ...
                return isImage ? (
                  ...
                  <a key={att.id} href={att.dataUrl} download={att.name}
                    style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 10px', background: 'var(--color-surface-raised)', borderRadius: 8, fontSize: 12, color: 'var(--color-muted-ash)', textDecoration: 'none' }}>
                    <Paperclip size={11} />
                    {att.name}
                  </a>
                );
          ...
          {/* Bill summary */}
          {update.billSummary && (
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 8, padding: '4px 10px', background: 'var(--color-surface-raised)', borderRadius: 8, fontSize: 12 }}>
