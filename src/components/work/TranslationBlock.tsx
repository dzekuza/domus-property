...
export default function TranslationBlock({ updateId, sourceText, translations, onTranslated }: Props) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
        ...
        {LANGS.map(l => (
          <button
            ...
            style={{
              ...
              background: selectedLang === l.code ? 'var(--color-surface-active)' : 'rgba(0,0,0,0.05)',
              color: selectedLang === l.code ? 'var(--color-midnight-ink)' : 'var(--color-muted-ash)',
      ...
      {translatedText && (
        <div style={{ marginTop: 8, padding: '10px 14px', background: 'var(--color-surface-raised)', borderRadius: 10, fontSize: 13, color: 'var(--foreground)', lineHeight: 1.5 }}>
          {translatedText}
        </div>
      )}
